...
- **Claude Code**: 注册一个 Stop 钩子 `wiki-sync hook-run` 到 `~/.claude/settings.local.json`（而非 `settings.json`），对话结束时 Claude Code 把事件（含 transcript 路径）从 stdin 传入，立即同步当前对话。写在 `settings.local.json` 是为了防止被 Claude Code 自身命令（`/effort`、`/status` 等）覆盖清掉——这个文件 Claude Code 永不自动写入。

...
flowchart LR
    ...
    B[Codex] --> S
    C[ChatGPT] --> S
    S["wiki-sync<br/>自动同步"] --> R["raw/wiki-sync-*<br/>对话原料"]
    R --> I["LLM-WIKI ingest<br/>自动编排"]
    I --> W["结构化 wiki<br/>项目知识"]
```
...
```bash
wiki-sync repair   # 按会话身份重建所有对话文件，互不覆盖
```

每个对话的文件名由它的会话身份唯一决定，因此**不同会话绝不会写同一个文件、互相覆盖**。如果你的库是早期版本（文件名只按标题，曾发生同名覆盖）建过的，跑一次 `repair`：它会从原始记录重新生成每个会话的独立文件、**补回此前被同名覆盖的对话**，并清理旧命名遗留文件。幂等，可反复运行。
...
wiki-sync 的产物**全部自包含在 `raw/` 下**、统一 `wiki-sync-*` 命名：每个 agent 一个文件夹、外加一份同步日志。文件夹里**只放对话、不生成任何索引**（Obsidian 自己负责索引和搜索，手维护的索引只会过时添乱），且**绝不写进 `wiki/` 等你亲手整理的区域**。凡是往库里写东西都会在 `raw/wiki-sync-log.md` 留一条痕——手动 `sync` 记一条运行汇总（`sync | 新增/跳过/失败` + 清单），自动钩子每轮写库记一条（`hook | 新增/更新`）。一次写库就是一次操作，操作必留痕：

```
你的 vault/
├── raw/
│   ├── wiki-sync-claude/
│   │   └── claude-<标题>-<id>.md  # 一个对话一个文件（<id> 由会话身份生成，不同会话绝不重名）
│   ├── wiki-sync-codex/
│   │   └── ...
│   ├── wiki-sync-opencode/      # 自定义 agent 也各自一个文件夹
│   └── wiki-sync-log.md         # 每次写库追加一条（手动 sync：新增/跳过/失败+清单；自动钩子：新增/更新）
...
- **Claude Code**: 注册一个 Stop 钩子 `wiki-sync hook-run` 到 `~/.claude/settings.local.json`（而非 `settings.json`），对话结束时 Claude Code 把事件（含 transcript 路径）从 stdin 传入，立即同步当前对话。写在 `settings.local.json` 是为了防止被 Claude Code 自身命令（`/effort`、`/status` 等）覆盖清掉——这个文件 Claude Code 永不自动写入。钩子还会**自愈**（万一被清掉会自动装回），但一旦你 `uninstall`，就会记下「已关闭」的意图、不再自愈——卸载是稳的，不会被自愈偷偷复活。
